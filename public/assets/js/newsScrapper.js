$(document).ready(function() {
  ...
  $(".searchArticle").on("click", () => {
    ...
    fetch("/api/search", { method: "GET" }).then(() =>
  ...
  $(".addArticle").on("click", function(element) {
    ...
    fetch("/api/add", {
      ...
      $("#modalMessage .modal-content ").html(
        "<h4> Sucessfully Added Article </h4>"
      );
  ...
  $(".savedArticles").on("click", () => {
    ...
    fetch("/api/savedArticles", { method: "GET" })
      ...
      .then(response => {
        response.map(article => {
          let articleDiv =
            ...
            "' data-slug='" +
            article.slug +
            "' class='collection-item avatar hover modal-trigger' href='#articleModal'><img src='" +
            article.imageURL +
            "'class='circle'><span class='title'>" +
            article.headline +
            "</span><p>" +
            article.summary +
            "</P><a class='secondary-content deleteArticle'><i class='material-icons hoverRed'>delete_forever</i></a></li>";
          ...
          $(document.getElementById(article["_id"])).on("click", function(
            event
          ) {
              ...
              .then(data => {
                ...
                if (data.length >= 1) {
                  data.map(comment => {
                    if (comment === null) {
                      let notesDiv =
                        "<div class='col s12 m7'><div class='card horizontal'><div class='card-image'><img src='https://lorempixel.com/100/190/nature/6'></div><div class='card-stacked center'><div class='card-content valign-wrapper'><p>No Notes.</p></div></div></div></div>";
                      $(".boxComments").prepend(notesDiv);
                    } else {
                      let notesDiv =
                        "<div class='col s12 m7' id='" +
                        comment["_id"] +
                        "'><div class='card horizontal'><div class='card-image'><img src='https://lorempixel.com/100/190/nature/6'></div><div class='card-stacked center'><div class='card-content valign-wrapper'><p>" +
                        comment.body +
                        "</p></div><div class='card-action deleteComment' data-id=" +
                        comment["_id"] +
                    ...
                    $(".deleteComment").on("click", function() {
                      ...
                      fetch("/api/deleteComment", {
                        ...
                        $(document.getElementById(comment["_id"])).css(
                          "display",
                          "none"
                        );
                      });
          ...
          $(".deleteArticle").on("click", function(event) {
            ...
            fetch("/api/deleteArticle", {
              ...
              $("#modalMessage").modal("open");
              $("#modalMessage .modal-content ").html(
                "<h4> Sucessfully Deleted:" + sessionArticle["_id"] + "</h4>"
              );
              setTimeout(() => $("#modalMessage").modal("close"), 2000);
              $(document.getElementById(sessionArticle["_id"])).css(
                "display",
                "none"
              );
            });
